...
% Collapse main cross sections
ES = data.Acceleration.Info.ErrorShape;
data.Neutronics.numberEnergyGroups = 1;
data.Neutronics.DiffusionXS = data.XS(2).DiffXS;
data.Neutronics.TotalXS = data.XS(2).TotalXS;
data.Neutronics.AbsorbXS = data.XS(2).AbsorbXS;
data.Neutronics.ScatteringXS = zeros(data.problem.NumberMaterials,1,1);
for m=1:data.problem.NumberMaterials
    data.Neutronics.ScatteringXS(m,1,1) = sum(squeeze(data.XS(1).ScatteringXS(m,:,:))*ES(m,:)');
end
