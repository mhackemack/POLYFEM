...
% ------------------------------------------------------------------------------
itmax = 1e8; tol = 1e-9; tol_SI=1e-14;
perform_SI = true; max_SI_fast=10000; max_SI_thermal=2;
add_within = false;
    ...
    for g=fg(1):fg(end)
        if perform_SI
            ...
            for k=1:max_SI_fast
                ...
                b = q(g) +dot(S(g,:,1),sol);
                sol(g) = b / T(g,g);
                if( abs(sol(g)-sg_old) < tol_SI )
                    convergence=true;
                    break
                end
            end
            resi_within(g) = S(g,g,1)*(sol(g)-sg_old);
            if ~convergence, fprintf('SI not converged in group %d \n',g); end
        else
            b = q(g) - dot(A(g,:),sol);
            delta = A(g,g)\b;
            sol(g) = sol(g) + delta;
