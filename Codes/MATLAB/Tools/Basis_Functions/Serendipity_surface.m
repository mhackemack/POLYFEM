...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = Serendipity_surface(verts, fverts, flags, deg, nverts)
nin = nargin;
nout = nargout;
%
...
if ctype == 1
    ...
    J = evaluate_numerical_jacobian(dim, verts, fverts, qqx, qx, ndim);
    invJ = cell(nqx,1);
    if dim == 3
        detJ = zeros(nqx,1);
        for q=1:nqx
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function out = evaluate_numerical_jacobian(dim, verts, fverts, x, xx, ind)
n = size(xx,1);
out = cell(n,1);
% Evaluate Jacobian
if dim == 2
...
if dim == 1 || ctype == 2
    out = 0;
    for d=0:min(dim,floor(deg/2))
        out = out + 2^(dim-d)*nchoosek(dim, d)*nchoosek(deg-d,d);
    end
else
    if dim == 2
        if deg == 3
...
out = get_serendipity_function('vals', ctype, dim);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function out = get_basis_grad_funcs(ctype, dim)
out = get_serendipity_function('grads', ctype, dim);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function out = eval_quad_grads_for_jac(xx)
n = size(xx,1); out = zeros(4,2,n);
for i=1:n
    s = xx(i,1); t = xx(i,2);
