%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   Title:          Wachpress Basis Function Generator - Volume Integrals
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [varargout] = wachspress_volume(varargin)
...
% ----------------------
M = zeros(nverts,nverts);       % mass matrix
K = zeros(nverts,nverts);       % stiffness matrix
G = cell(dim,1);                % gradient matrix
for i=1:dim
    G{i} = zeros(nverts,nverts);
end
% Get Basis Function Values and Gradients
% ---------------------------------------
[qx, qw] = get_general_volume_quadrature(verts, faces, 2*order+1, true); nqx = length(qw);
if flags(2) || flags(3)
    [bme, gme] = wachpress_basis_functions(verts, qx, faces, order, nverts);
else
    bme = wachpress_basis_functions(verts, qx, faces, order, nverts);
end
