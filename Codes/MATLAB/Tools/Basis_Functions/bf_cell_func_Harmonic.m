%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   Title:          Harmonic Main Generation Function
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = bf_cell_func_Harmonic( varargin )
...
% ------------------------------------------------------------------------------
if order > 1 , error('Harmonic only defined for order 1.'); end
...
[bmv, gmv] = harmonic_basis_functions(verts, qx_v, faces, order, nverts);
% mass matrix
for q=1:nqx
    bt = bmv(q,:);
    M = M + qw_v(q) * (bt'*bt);
...
% Face-Wise Values
[qx_s, qw_s, bms, gms] = get_surface_values(dim, verts, faces, ord);
for f=1:nf
    nqx = length(qw_s{f});
    for q=1:nqx
        bt = bms{f}(q,:);
        MM = MM + qw_s{f}(q) * (bt'*bt);
        if s_flags(2)
