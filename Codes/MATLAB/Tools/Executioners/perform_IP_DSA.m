%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   Title:          Solve IP DSA Diffusion Problem
%
...
%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = perform_IP_DSA(ndat, solvdat, mesh, DoF, FE, x, A)
...
for f=1:mesh.TotalFaces
    ...
    if fflag == 0
        ...
        for g=1:ndat.numberEnergyGroups
            ...
            % ------------------------------------------------------------------
            % (-,-)
            L(gfnodes1,gfnodes1) = L(gfnodes1,gfnodes1) + kp*M{1};
            % (+,+)
            L(gfnodes2,gfnodes2) = L(gfnodes2,gfnodes2) + kp*M{2};
            % (+,-)
            L(gfnodes2,gfnodes1) = L(gfnodes2,gfnodes1) - kp*MM{2};
            % (-,+)
            L(gfnodes1,gfnodes2) = L(gfnodes1,gfnodes2) - kp*MM{1};
            % Gradient Terms
            % ------------------------------------------------------------------
            % (-,-)
            L(gcnodes1,gcnodes1) = L(gcnodes1,gcnodes1) - 0.5*D(1)*(G{1} + G{1}');
            % (+,+)
            L(gcnodes2,gcnodes2) = L(gcnodes2,gcnodes2) + 0.5*D(2)*(G{2} + G{2}');
            % (+,-)
            L(gcnodes2,gcnodes1) = L(gcnodes2,gcnodes1) + 0.5*(D(1)*CG{1}' - D(2)*CG{2});
            % (-,+)
            L(gcnodes1,gcnodes2) = L(gcnodes1,gcnodes2) - 0.5*(D(2)*CG{2}' - D(1)*CG{1});
        end
    % Boundary Face
    else
...
for f=1:mesh.TotalFaces
    ...
    if fflag == 0
        ...
        for g=1:ndat.numberEnergyGroups
            ...
            % ------------------------------------------------------------------
            % (-,-)
            out(gfnodes1) = out(gfnodes1) + kp*M{1}*x(gfnodes1);
            % (+,+)
            out(gfnodes2) = out(gfnodes2) + kp*M{2}*x(gfnodes2);
            % (+,-)
            out(gfnodes2) = out(gfnodes2) - kp*MM{2}*x(gfnodes1);
            % (-,+)
            out(gfnodes1) = out(gfnodes1) - kp*MM{1}*x(gfnodes2);
            % Gradient Terms
            % ------------------------------------------------------------------
            % (+,+)
            out(gcnodes2) = out(gcnodes2) + 0.5*D(2)*(G{2} + G{2}')*x(gcnodes2);
            % (-,-)
            out(gcnodes1) = out(gcnodes1) - 0.5*D(1)*(G{1} + G{1}')*x(gcnodes1);
            % (+,-)
            out(gcnodes2) = out(gcnodes2) + 0.5*(D(1)*CG{1}' - D(2)*CG{2})*x(gcnodes1);
            % (-,+)
            out(gcnodes1) = out(gcnodes1) - 0.5*(D(2)*CG{2}' - D(1)*CG{1})*x(gcnodes2);
        end
    % Boundary Face
    else
        ...
        for g=1:ndat.numberEnergyGroups
            ...
            if     (ndat.Transport.BCFlags(fflag) == glob.Vacuum || ...
                    ...
                    ndat.Transport.BCFlags(fflag) == glob.IncidentCurrent || ...
                    ndat.Transport.BCFlags(fflag) == glob.IncidentBeam)
%                 out(gfnodes) = out(gfnodes) + 2*kp*M*x(gfnodes);
