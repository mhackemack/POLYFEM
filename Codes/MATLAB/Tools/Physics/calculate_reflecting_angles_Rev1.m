...
%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = calculate_reflecting_angles_Rev1( data, mesh )
global glob
dim = mesh.Dimension;
% Calculate 1D Reflecting Angles
if dim == 1
    for q=1:length(data.Quadrature)
        data = calculate_1D_reflecting_angles(data, q, mesh);
    end
    return
end
for q=1:length(data.Quadrature)
    na = data.Quadrature(q).NumberAngularDirections;
    octAngles = na / 2^dim;
    octNumbers = get_octant_numbers( na, mesh.Dimension );
    data.Quadrature(q).ReflectingBoundaryAngles = cell(mesh.TotalFaces, 1);
    mins = get_mins(dim);
    % Loop through boundary faces
    for f=1:mesh.TotalBoundaryFaces
        ff = mesh.BoundaryFaces(f);
        fid = mesh.FaceID(ff);
        nflag = data.XS(data.Transport.XSID).BCFlags(fid);
        if nflag ~= glob.Reflecting, continue; end
        data.Quadrature(q).ReflectingBoundaryAngles{ff} = zeros(na, 1);
        fnorm = mesh.FaceNormal(ff,:);
        for f_out=1:na
            angDir = data.Quadrature(q).AngularDirections(f_out,:);
            fdot = angDir * fnorm';
            if abs(fdot) < glob.small, continue; end
            if fdot > 0
                f_in = get_reflected_angle(fnorm, f_out, octNumbers(f_out), octAngles, mins);
                data.Quadrature(q).ReflectingBoundaryAngles{ff}(f_out) = f_in;
                data.Quadrature(q).ReflectingBoundaryAngles{ff}(f_in) = f_out;
            end
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = calculate_1D_reflecting_angles(data, q, mesh)
global glob
data.Quadrature(q).ReflectingBoundaryAngles = cell(mesh.TotalFaces, 1);
na = data.Quadrature(q).NumberAngularDirections;
% Left Boundary
...
if nflag == glob.Reflecting
    data.Quadrature(q).ReflectingBoundaryAngles{1} = zeros(na, 1);
    c = na;
    for m=1:na/2
        data.Quadrature(q).ReflectingBoundaryAngles{1}(m) = c;
        data.Quadrature(q).ReflectingBoundaryAngles{1}(c) = m;
        c = c - 1;
    end
end
% Right Boundary
...
nflag = data.XS(data.Transport.XSID).BCFlags(fid);
if nflag == glob.Reflecting
    data.Quadrature(q).ReflectingBoundaryAngles{end} = zeros(na, 1);
    c = na/2;
    for m=na/2+1:na
        data.Quadrature(q).ReflectingBoundaryAngles{end}(m) = c;
        data.Quadrature(q).ReflectingBoundaryAngles{end}(c) = m;
        c = c - 1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
