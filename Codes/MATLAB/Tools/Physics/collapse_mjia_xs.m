...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = collapse_mjia_xs(data,xsin,xsout,accid)
global glob
...
for m=1:nm
    S = squeeze(sxs(m,:,:));
    A = (diag(txs(m,:)) - diag(diag(S)))\(tril(S,-1) + triu(S,1));
...
% ------------------------------------------------------------------------------
if data.Acceleration.Info(accid).AccelerationType == glob.Accel_WGS_MJIA_DSA
