...
% ------------------------------------------------------------------------------
func_P0_noaccel = get_MTG_fourier_func('unaccelerated',0);
func_P0_accel   = get_MTG_fourier_func('accelerated',0);
% func_P1_noaccel = get_MTG_fourier_func('unaccelerated',1);
% func_P1_accel   = get_MTG_fourier_func('accelerated',1);
% ------------------------------------------------------------------------------
...
%     P1_noaccel_out{1,m+1} = matname; P1_accel_out{1,m+1} = matname;
    ...
    A = (T(tg,tg) - tril(S(tg,tg,1),-1))\triu(S(tg,tg,1),0);
    [V,D] = eig(A); D = diag(D);
    [eval,Ei] = max(abs(D)); D = [];
...
% Print output files
cell2csv([glob.output_path,data.OutputName,'/Fourier_MTG_P0_noaccel'],P0_noaccel_out);
cell2csv([glob.output_path,data.OutputName,'/Fourier_MTG_P0_accel'],P0_accel_out);
% cell2csv([glob.output_path,data.OutputName,'/Fourier_MTG_P1_noaccel'],P1_noaccel_out);
% cell2csv([glob.output_path,data.OutputName,'/Fourier_MTG_P1_accel'],P1_accel_out);
